...
export default function LoginPage() {
  ...
  const handle = async () => {
    ...
    try { const { token } = await auth.login(email, password); setToken(token); router.push("/dashboard"); }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-6 bg-[var(--background)]">
      <div className="max-w-md mx-auto w-full">
        ...
        <h1 className="text-3xl font-extrabold text-[var(--foreground)] font-[family-name:var(--font-anybody)] mb-1">Welcome back</h1>
        <p className="text-[var(--muted-foreground)] text-sm mb-8">Log in to continue.</p>
        ...
        <div className="flex flex-col gap-3">
          ...
          <input value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" type="password" className={inp} onKeyDown={e => e.key === "Enter" && email && password && handle()} />
        </div>
        <div className="text-right mt-3">
          <Link href="/forgot-password" className="text-[var(--muted-foreground)] text-xs hover:text-[var(--primary)] transition-colors">Forgot password?</Link>
        </div>
        <button onClick={handle} disabled={!email || !password || loading}
          className="w-full mt-5 bg-[var(--primary)] text-[var(--primary-foreground)] font-extrabold py-4 rounded-xl text-sm font-[family-name:var(--font-anybody)] hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-35">
          {loading ? "LOGGING IN..." : "LOG IN"}
        </button>
        <GoogleButton onSuccess={handleGoogle} />
        <p className="text-center text-[var(--muted-foreground)] text-sm mt-6">No account? <Link href="/signup" className="text-[var(--primary)] hover:underline">Sign up</Link></p>
      </div>
    </div>
  );
}
